...
class TFFashionMNIST(object):
    ...
    def create_model(self):
        ...
        with tf.name_scope('conv1'):
            self.conv1 = tf.layers.conv2d(inputs=self.data, kernel_size=(3, 3), filters=30)
            # self.conv11 = tf.layers.conv2d(inputs=self.conv1, kernel_size=(3, 3), filters=30)
            self.relu1 = tf.nn.relu(self.conv1)
            self.mpool1= tf.layers.max_pooling2d(inputs=self.relu1, pool_size=(2, 2), strides=(2, 2))
            # potential dropout

        with tf.name_scope('conv2'):
            self.conv2 = tf.layers.conv2d(inputs=self.mpool1, kernel_size=(3, 3), filters=60)
            # self.conv22 = tf.layers.conv2d(inputs=self.conv2, kernel_size=(3, 3), filters=60)
            self.relu2 = tf.nn.relu(self.conv2)
            self.mpool2= tf.layers.max_pooling2d(inputs=self.relu2, pool_size=(2, 2), strides=(2, 2))

        with tf.name_scope('conv3'):
            self.conv3 = tf.layers.conv2d(inputs=self.mpool2, kernel_size=(3, 3), filters=120)
            # self.conv3 = tf.layers.conv2d(inputs=self.conv3, kernel_size=(3, 3), filters=120)
            self.relu3 = tf.nn.relu(self.conv3)
            self.mpool3= tf.layers.max_pooling2d(inputs=self.relu3, pool_size=(2, 2), strides=(2, 2))
